...
int gr_torus_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new Torus(innerRadius, outerRadius));
...
int gr_cone_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new Cone(radius, height));
...
int gr_plane_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new Plane(width, height));
...
extern "C"
int gr_AA_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  double AA = luaL_checknumber(L, 1);
  Renderer::AA_VALUE = AA;
  return 0;
}
...
{
  GRLUA_DEBUG_CALL;
  double DOF = luaL_checknumber(L, 1);
  double dispersion = luaL_checknumber(L, 2); 
  double numThreads = luaL_checknumber(L, 3);
  Renderer::set_dof(DOF, dispersion, numThreads);
  return 0;
}

extern "C"
int gr_cel_shading_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  bool index = lua_toboolean(L, 1);
  Renderer::CELLSHADING_ON = index;
  return 0;
}

extern "C"
int gr_multi_threads_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  double index = luaL_checknumber(L, 1);
  Renderer::NUM_THREADS = index;
  return 0;
}
...
int gr_node_set_texture_cmd(lua_State* L)
{
  ...
  if (!texture->loadPng(name)) { std::cerr << "LUA:Loading PNG Failed!!" << std::endl; return -1; }
...
static const luaL_reg grlib_functions[] = {
  {"AA" , gr_AA_cmd},
  {"DOF", gr_DOF_cmd},
  {"cel_shading", gr_cel_shading_cmd},
  {"num_threads", gr_multi_threads_cmd},
  {"node", gr_node_cmd},
...
static const luaL_reg grlib_node_methods[] = {
  ...
  {"render", gr_render_cmd},
  {"set_km", gr_node_set_km_cmd},
  {"set_refractionIndex", gr_node_set_refractionIndex_cmd},
  {0, 0}
};
